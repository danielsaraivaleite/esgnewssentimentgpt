...
def recupera_lista_empresas_B3():
    ...
    if 'Código' not in list(df.columns):
        df = df.rename(columns={'Ticker' : 'Código'})
    
    df = df.loc[:, ['Código', 'Nome']].sort_values(by='Nome')
    ...
    new_row = {'Código':'Braskem', 'Nome': 'Braskem' }
    df = df.append(new_row, ignore_index=True) 
...
def processa_busca_empresa(empresa, dfEmpresasListadas, atualiza=False, apenas_titulos=False, aplicar_filtros=False):
    '''
